...
* Create a project S3 bucket using the AWS Cloudshell
    ...
    ```sh
    aws s3 mb s3://thailx1-project4-dend/
    ```
...
* Copy the data from the udacity bucket to the home cloudshell directory: 
    ...
    ```sh
    aws s3 cp ~/log-data/ s3://thailx1-project4-dend/log-data/ --recursive
    aws s3 cp ~/song-data/ s3://thailx1-project4-dend/song-data/ --recursive
    ```

* List the data in bucket to be sure it copied over: 

    ```sh
    aws s3 ls s3://thailx1-project4-dend/log-data/
    aws s3 ls s3://thailx1-project4-dend/song-data/
    ```
...
<img src="./images/create-tables_graph.png">
...
<img src="./images/final-project-graph.png" width=90% height=50%>
...
* `Key` = `s3_bucket`
* `Value` = `thailx1-project4-dend` (bucket name)
